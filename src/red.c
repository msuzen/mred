...
int main()
{
   ...
   double real_time,delta_time=0.01;
    ...
    printf("  read network file !\n"); 
     ...
     nran=(int *) malloc((simtime+1)*(numhost+1)*sizeof(int)); /* ramdom zeros and ones */
     nran2=(int *) malloc((numhost+1)*sizeof(int)); /* choosing host during each call */
   ...
   for(i=0;i<numhost;i++) {
     ...
     /* core RED algorithm */
      ...
      for(time=0;time<simtime; time++) {
            ...
            for(i=0;i<numhost;i++) {
                   nran2[i]=nran[rcount];
                    /* printf("time %d host %d sending ? %d\n",time,i,nran2[i]);  */
               if(nran[rcount] == 1) {
                  q=q+hostrate[i];
	     ...
	     printf("core RED Algorithm time =%d line1=%s \r",time,line1); 
               // if(nran2[0] == 1) { printf("here bonur 1st sending \n"); }
	      /*  printf("line %s\n",line1);*/
              fprintf(fp3,"%s\n",line1);
               q_time[time]=q;
